...
const template = (city, type) => {
  ...
  const service = type === 'totaalrenovatie' ? 'Totaalrenovatie' : 'Renovatie';
  const descPrefix = type === 'totaalrenovatie' 
    ? `Totaalrenovatie ${cityName}: Kosten, soorten en diensten - Alles voor uw renovatie.`
    : `Renovatie ${cityName} | Totaalrenovatie & Gevelwerken | Yannova`;

  const keywords = [`${type} ${cityName}`, `renovatiewerken ${cityName}`, `aannemer ${cityName}`, `verbouwen ${cityName}`, `renovatiebedrijf ${cityName}`];
...
export const metadata: Metadata = generateSEO({
  title: "${descPrefix}",
  description: "${service} in ${cityName}? Yannova is dé specialist voor een zorgeloze verbouwing van A tot Z. Transparante kosten, 15+ jaar ervaring en 1 vast aanspreekpunt. Vraag een offerte!",
  path: "/${type}/${city.toLowerCase()}",
...
const breadcrumbSchema = generateBreadcrumbSchema([
  ...
  { name: "${service}", url: "https://www.yannova.be/${type}" },
  { name: "${service} ${cityName}", url: \`https://www.yannova.be/${type}/${city.toLowerCase()}\` },
]);
...
export default function ${service}${cityName.replace(/[^a-zA-Z]/g, '')}Page() {
  return (
    <>
      <script
          ...
          <div className="container relative z-10 mx-auto px-6">
            <div className="max-w-4xl">
              ...
              <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-white mb-6 leading-tight">
                ${service} in <span className="text-secondary">${cityName}</span>
              </h1>
              <p className="text-xl text-gray-300 leading-relaxed mb-8 max-w-2xl">
                Bent u op zoek naar een betrouwbare aannemer voor een ${service.toLowerCase()} in ${cityName}? 
          ...
          <div className="lg:col-span-2 space-y-16">
            ...
            <section>
              <h2 className="text-3xl font-bold mb-6 text-gray-900">
                Alles voor uw renovatieproject in ${cityName}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-6 text-lg">
                Een ${service.toLowerCase()} vraagt om een strakke coördinatie. Bij Yannova werken specialisten 
                uit verschillende vakgebieden nauw samen. Hierdoor verloopt uw verbouwing in ${cityName} vlot en zonder 
            ...
            <section className="bg-gray-100 -mx-6 px-6 py-12 rounded-none md:rounded-2xl">
              <h2 className="text-3xl font-bold mb-6 text-gray-900">
                Uw voordelen bij Yannova in ${cityName}
              </h2>
              <p className="text-gray-700 leading-relaxed mb-8 text-lg">
                Wij geloven dat een ${service.toLowerCase()} geen nachtmerrie hoeft te zijn. Door te kiezen voor 
              ...
              <ul className="space-y-4">
                <li className="flex items-start gap-4 bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm mt-1">U heeft één vaste projectleider voor uw werf in ${cityName}. Dat betekent duidelijke communicatie en snelle beslissingen.</p>
                  </div>
                </li>
                <li className="flex items-start gap-4 bg-white p-4 rounded-lg shadow-sm">
                  <ShieldCheck className="w-6 h-6 text-secondary flex-shrink-0 mt-1" />
                ...
                <li className="flex items-start gap-4 bg-white p-4 rounded-lg shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm mt-1">Ontdek uw recht op verbouwpremies of renovatiesubsidies in ${cityName}. Wij leveren de correcte attesten af.</p>
                  </div>
                </li>
              </ul>
            </section>
            ...
            <section>
              <h2 className="text-3xl font-bold mb-6 text-gray-900">
                Renoveren in ${cityName}: Lokale regelgeving en subsidies
              </h2>
              <p className="text-gray-700 leading-relaxed mb-6 text-lg">
                Voor grote renovatiewerken aan de buitenschil van uw woning of constructieve werken, heeft u in ${cityName} 
              ...
              <div className="bg-blue-50 border-l-4 border-secondary p-6 rounded-r-xl">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="text-xl font-bold text-gray-900">Vlaamse Mijn VerbouwPremie in ${cityName}</h3>
                </div>
                <p className="text-gray-700 leading-relaxed text-sm">
                  ...
                  voor de Mijn VerbouwPremie? Voor inwoners van ${cityName} kan een renovatieproject tot tienduizenden euro's voordeliger uitvallen 
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-28 bg-gray-900 rounded-2xl p-8 text-white shadow-xl">
              <h3 className="text-2xl font-bold mb-4">
                Droomt u van een vernieuwde woning in ${cityName}?
              </h3>
              <p className="text-gray-300 mb-8 leading-relaxed text-sm">
                Vraag een vrijblijvend plaatsbezoek aan. Wij komen ter plekke in ${cityName} om uw wensen te bespreken, 
              ...
              <div className="mt-8 pt-8 border-t border-white/10">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-400">Uw aannemer in ${cityName}.</div>
                  </div>
                </div>
              </div>
            </div>
...
console.log('🚀 Generating local SEO pages for Renovatie & Totaalrenovatie...\n');
let created = 0;
let overwritten = 0;

['renovatie', 'totaalrenovatie'].forEach(type => {
  municipalities.forEach(city => {
    const dir = path.join(process.cwd(), 'src', 'app', '(public)', type, city);
    ...
    fs.writeFileSync(filePath, template(city, type));
    console.log(`✅ Generated: /${type}/${city}`);
  });
});

console.log(`\n📊 Summary Renovatie/Totaalrenovatie:`);
