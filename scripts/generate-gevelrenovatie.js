...
const template = (city) => `import { Metadata } from 'next';
...
municipalities.forEach(city => {
  const dirPath = path.join(__dirname, '..', 'src', 'app', '(public)', 'gevelrenovatie', city);
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }
  fs.writeFileSync(path.join(dirPath, 'page.tsx'), template(city));
  console.log("Created gevelrenovatie page for " + city);
});
